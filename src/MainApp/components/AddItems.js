...
export default function AddItems({
  ...
  const pushItem = async () => {
    ...
    const item = await axios
      .post(`${remoteBackend}storeV1`, formdata, config)
      .catch((err) => {
        console.log(err);
  ...
  const getSeverItem = async () => {
    const data = await axios.get(`${remoteBackend}storeV1`).catch((err) => {
      console.log(err);
      const refinedErr = JSON.stringify(err);
      console.log(JSON.parse(refinedErr));
      console.log(JSON.parse(refinedErr.response));
  ...
  const loaderIcon = (
    <svg
      role="status"
      class="mr-2 w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-green-400"
      viewBox="0 0 100 101"
      fill="none"
  ...
  const uploadIcon = (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="group-hover:fill-green-400 fill-white"
      viewBox="0 0 24 24"
      width="24"
      height="24"
    >
  ...
  return (
    <div className="flex items-center justify-center flex-col">
      <div className="w-48 border border-gray-300 p-2 rounded-lg bg-gray-200">
        <p className="text-sm font-bold mb-2 text-gray-700 uppercase flex justify-center items-center">
          {userIcon}
          {user.name}'s ITEM CARD
        </p>
        ...
        <form className="flex flex-col justify-center items-center w-full">
          <label
            htmlFor="img"
            className={`bg-gray-700 rounded mb-4 py-2 px-3 w-full text-white hover:bg-gray-900`}
          >
            {" "}
            {itemInput.result ? imageLoadedText : "Click to input image"}
          </label>
          <input
            type="file"
            id="img"
            ...
            className={`ml-2 mb-4 text-sm hidden `}
            accept="image/*"
            name="img"
          ...
          <input
            ...
            className="mb-4 focus:outline outline-gray-400 outline-2 bg-gray-100 outline-gray-300 rounded placeholder:text-gray-500 p-2 w-full"
            placeholder="Name"
          ></input>
          <input
            type="number"
            ...
            className="bg-gray-100 mb-4 focus:outline outline-2 outline-gray-400 p-2 placeholder:text-gray-500 rounded w-full"
            placeholder="Price"
          ></input>
          <textarea
            ...
            className="bg-gray-100 p-2 outline-gray-300 focus:outline outline-2 outline-gray-400 placeholder:text-gray-500 rounded w-full"
      ...
      <div className="flex justify-center items-center">
        <button
          className="px-3 py-1 mr-2 w-24 bg-green-400 text-xs rounded-sm font-bold mt-2 hover:bg-black hover:text-green-400 uppercase text-white"
          onClick={() => {
        ...
        <button
          className="px-3 flex justify-center items-center group py-2 w-24 bg-green-400 text-xs rounded-sm font-bold mt-2 hover:bg-black hover:text-green-400 uppercase text-white"
          onClick={() => {
            pushItem();
          }}
        >
      ...
      {postRequest ? <div className="mt-2">{loaderIcon}</div> : null}

      {errorMessage && (
        <p className="text-red-500 text-xs font-bold">{errorMessage}</p>
      )}
      {goodPost && (
        <p className="flex uppercase text-xs font-bold justify-center items-center mt-2 text-gray-700">
          Saved succesful<p className="ml-1">{saveIcon}</p>
        </p>
      )}
    </div>
  );
