...
export default function CrudItem({
  ...
  const updateSingle = async (e) => {
    ...
    const patchedItem = await axios
      .patch(`${remoteBackend}storeV1/${id}`, itemUpdate, config)
      .catch((err) => {
        console.log(err);
        return;
      });
  ...
  const deleteRequest = async (e) => {
    ...
    const item = await axios
      .delete(`${remoteBackend}storeV1/${id}`)
      .catch((err) => {
        console.log(err);
      });
    setItemData((prev) => {
  ...
  const loaderIcon = (
    <svg
      role="status"
      class="mr-2 w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-green-400"
      viewBox="0 0 100 101"
      fill="none"
  ...
  const uploadIcon = (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className={`${
        updateGlow ? "fill-green-400" : "fill-gray-600"
      } hover:fill-black`}
      viewBox="0 0 24 24"
      width="18"
      height="18"
  ...
  return (
    <div className="flex max-w-screen px-2 pb-2">
      <div className="">
        <input
          id="img"
          name="img"
        ...
        <label htmlFor="img" className="h-10">
          <img
            ...
            className="h-10 sm:h-15 sm:w-8 w-8 rounded-l-lg border-green-400 border border-r-gray-300"
          ></img>
      ...
      <input
        ...
        className="text-gray-700 p-1 focus:bg-gray-200 bg-gray-100 outline-none text-center focus:outline border-y w-12 sm:w-32 text-xs border-green-400 overflow-hidden h-10"
      ></input>
      ...
      <input
        ...
        className="text-gray-700 focus:bg-gray-200 bg-gray-100 text-center outline-none focus:outline border sm:w-32 border-r-gray-300 border-l-gray-300 border-green-400 text-xs w-12 overflow-hidden h-10"
      ></input>
      ...
      <textarea
        ...
        className="text-gray-700 p-1 focus:bg-gray-200 outline-none bg-gray-100 focus:outline border-y sm:w-32 border-r text-center border-green-400 w-16 overflow-hidden text-xs h-10 rounded-r-lg"
      ></textarea>
