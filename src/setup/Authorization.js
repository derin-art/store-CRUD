...
export default function Authorization() {
  ...
  const [backgroundImageDetails, setBackGroundPhoto] = React.useState({
    backGroundPhoto: defaultImg,
    backGroundPhotoAuthor: "Image by coporation",
  });
  ...
  const getBackGroundImages = async () => {
    ...
    const data = await fetch(
      `https://apis.scrimba.com/unsplash/photos/random?orientation=${param}&query=ocean`
    );
  ...
  const createUser = async () => {
    ...
    const data = await axios
      .post(`${remoteBackend}users`, {
        name: registerFormData.name,
        email: registerFormData.email,
        password: registerFormData.setPassword,
      })
      .catch((err) => {
  ...
  const signIn = async () => {
    ...
    const res = await axios
      .post(`${remoteBackend}users/login`, {
  ...
  const registerForm = (
    <form className="flex flex-col pl-2 sm:items-center sm:justify-center">
      <input
        className="p-2 pr-0 w-32 mt-4 text-sm border rounded outline-none focus:bg-gray-100 text-gray-600"
        placeholder="name"
        name="name"
      ...
      <input
        ...
        name="setPassword"
        value={registerFormData.setPassword}
        onChange={(event) => {
          updateRegisterForm(event);
        }}
      ></input>
      <input
        ...
        name="checkPassword"
        value={registerFormData.checkPassword}
        onChange={(event) => {
          updateRegisterForm(event);
        }}
      ></input>
      <button
        type="button"
        onClick={() => {
          validateForm(registerFormData);
        }}
        className="w-24 pr-0 mt-4 p-2 text-blue-400 border border-blue-900 focus:bg-blue-700 focus:text-white hover:bg-gray-900 "
      >
        register
      </button>
    </form>
  ...
  const signInForm = (
    <form className="flex flex-col pl-2 sm:items-center sm:justify-center">
      <input
        className="p-2 w-24 mt-4 text-sm border rounded outline-none focus:bg-gray-100 text-gray-600"
        type="email"
        placeholder="email"
        name="email"
      ...
      ></input>
      <input
        className="p-2 w-24 mt-4 text-sm border rounded outline-none text-gray-600 focus:bg-gray-100"
        type="password"
      ...
      <button
        ...
        className="w-24 mt-4 p-2 text-blue-400 border border-blue-900 focus:bg-blue-700 focus:text-white hover:bg-gray-900 "
      >
        login
      </button>
    </form>
    ...
    <div>
      <Routes>
        <Route
          ...
          path="/"
          element={
            returnedCreatedUser.status === 200 ? (
              <Navigate to="/user" />
            ) : (
              <div className="h-screen  flex ">
                <div className="w-3/4 flex flex-col item-center">
                  <h1 className="text-xl pt-12 text-gray-700 text-center">
                    {register ? "Login" : "Register"}
                  </h1>
                  {register ? signInForm : registerForm}
                  <button
                    className="p-2 text-xs text-left mt-6 text-blue-900 sm:text-center"
                    onClick={() => {
                      setRegister((prev) => !prev);
                    }}
                  >
                    {register ? "don't have an account yet?" : "want to login?"}
                  </button>
                  {errorMessage ? (
                    <p className="text-xs text-red-400 sm:text-center text-left p-1">
                      {errorMessage}
                    </p>
                  ) : null}
                  {Loading ? (
                    <div className="flex pl-3 sm:pl-0 sm:justify-center sm:align-center">
                      {Loader}
                    </div>
                  ) : null}
                  <div className="text-center text-sm text-gray-700">
                    <span className="text-red-700 mr-2">Note:</span>
                    use email: <span className="text-blue-700">user</span> and
                    Password: <span className="text-blue-700">password</span>{" "}
                    for easy login
                  </div>
                </div>

                <div
                  className="w-3/4 h-screen bg-blue-100 overflow-x-hidden sm:w-full max-w-screen bg-cover"
                  style={{
                    backgroundImage: `url(${backgroundImageDetails.backGroundPhoto})`,
                  }}
                >
                  <div
                    className="flex flex-col p-1 pb-0 font-sans  inline text-shadow z-10 sm:absolute sm:top-6 "
                    style={{ textShadow: "2px 2px 5px gray" }}
                  >
                    <div className="text-4xl sm:text-6xl text-white text-shadow -ml-1 inline">
                      Discover
                    </div>
                    <div className="text-2xl sm:text-4xl text-white inline">
                      Découvrir
                    </div>
                    <div className="text-lg text-white inline">Descubrir</div>
                  </div>

                  <p className="text-xs font-sans text-right mr-2 text-white absolute bottom-10 ml-2">
                    photo by {backgroundImageDetails.backGroundPhotoAuthor}
                  </p>
                </div>
              </div>
            )
        ...
        <Route
          path="/user/*"
          element={<MainPage user={returnedCreatedUser} />}
        />
      </Routes>
    </div>
  );
